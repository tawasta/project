...
class InstallationChecklistItem(models.Model):
    ...
    task_id = fields.Many2one('project.task', string='Project Task', readonly=True, copy=False, index=True)
    ...
    def create(self, vals):
        ...
        if rec.installation_id:
            ...
            if project:
                ...
                description_parts.append("Installation: %s" % escape(rec.installation_id.display_name))
                if rec.module_id:
                    description_parts.append("Module: %s" % escape(rec.module_id.display_name))
                ...
                item_link = Markup(
                    "<a href='#' data-oe-model='installation.checklist.item' data-oe-id='%d'>%s</a>"
                ) % (rec.id, escape(item_display))

                task.message_post(
                    body=Markup(
                        _("Linked checklist item: %s<br/>Module: %s") % (
                            item_link, escape(module_name)
                        )
                    ),
